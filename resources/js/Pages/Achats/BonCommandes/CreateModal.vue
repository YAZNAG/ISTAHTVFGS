<template>
    <Modal ref="createMarcheModal" max-width="4xl">
        <!-- Header -->
        <div class="mb-2">
            <h2 class="text-lg font-semibold text-gray-900">Créer un Marché</h2>
        ...
        <form @submit.prevent="submitMarcheForm" class="p-6 space-y-6" enctype="multipart/form-data">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="marcheForm.date_fin" type="date" required :min="marcheForm.date_debut"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" />
            ...
            <div class="border-t pt-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" @click="addRow" :disabled="!marcheForm.categorie_id"
                        class="bg-green-600 text-white px-3 py-2 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        + Article manuel
                    </button>
                </div>

                <div v-for="(row, index) in marcheForm.articles" :key="index"
                    class="relative mb-4 border p-4 rounded-lg bg-gray-50 flex gap-4">
                    <!-- Article Search -->
                    <div class="relative">
                        <label class="block text-sm font-medium mb-1">Article</label>
                        <input v-model="row.search" type="text" placeholder="Rechercher un article..."
                            @focus="row.dropdownOpen = true" @blur="closeIdle(index)"
                            class="w-full border-gray-300 rounded-lg p-2" />
                        <ul v-if="row.dropdownOpen && filteredArticles(row.search).length"
                            class="absolute w-full z-10 bg-white border border-gray-300 rounded-md mt-1 shadow-lg max-h-[200px] overflow-y-auto">
                            <li v-for="article in filteredArticles(row.search)" :key="article.id"
                    ...
                    <button type="button" class="text-white bg-red-500 text-sm px-3 py-2 rounded self-end ml-auto"
                        @click="removeRow(index)">
                        Supprimer
                    </button>
                </div>

                <div v-if="marcheForm.articles.length === 0"
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t">
                <button type="button" @click="createMarcheModal.close()"
...
const props = defineProps({
    ...
    categories: { type: Array, default: () => [] },        // [{id,nom}]
    articles: { type: Array, default: () => [] },   // [{id,designation,unite_mesure}]
})
...
const marcheForm = useForm({
    ...
    categorie_id: '',
    articles: [], // {article_id,designation,unite_mesure,quantite_commandee,taux_tva,search,dropdownOpen}
})
...
function closeIdle(index) {
    // petite temporisation pour laisser le temps au click @mousedown
...
function submitMarcheForm() {
    ...
    .transform(data => {
        const articles = data.articles.map(article => {
            const {article_id, quantite_commandee, taux_tva} = article;

            return {article_id, quantite_commandee, taux_tva}
        })
...
<style scoped>
/* rien de spécial – Tailwind gère déjà l’apparence */
</style>