...
const approve = () => {
    approveForm.put(route('bon-sorties.approve', props.sortie), {
        preserveScroll: true,
        onSuccess: () => {
            approveForm.reset();
...
const reject = () => {
    approveForm.put(route('bon-sorties.reject', props.sortie), {
        preserveScroll: true,
        onSuccess: () => {
            approveForm.reset();
            approveModalRef.value.close();
...
<AuthenticatedLayout>
    ...
    <Modal ref="approveModalRef">
        <div class="mb-2">
            <h2 class="text-lg font-semibold text-gray-900">Détails de la sortie <span class="underline">{{ sortie.numero }}</span></h2>
        ...
        <div class="space-y-6">
            <div class="bg-white p-6 rounded-lg shadow-sm border">
                ...
                <div class="mt-4">
                    <p class="text-gray-500 text-sm">Demandeur:</p>
                    <span :class="['px-2 py-1 rounded text-sm font-medium', sortieType.color]">
                        {{ sortie.demandeur || '—' }}
                    </span>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item in sortie.articles" :key="item.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center">{{ item.quantite }} {{ item.unite_mesure }}</td>
