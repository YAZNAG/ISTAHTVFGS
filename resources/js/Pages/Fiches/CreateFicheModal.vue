...
const form = useForm({
    ...
    effectif: 1,
    demandeur: '',
    etapes: [],          // only { title, description }
    articles: [],         // only { article_id, designation, quantite, unite_mesure, prix_unitaire }
})
    ...
    <Modal ref="createFicheModal" size="3xl" class="overflow-y-scroll">
        ...
        <div class="flex justify-end space-x-3 pt-4">
            <button type="button" @click="$emit('close')"
                class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Annuler</button>
            <button type="button" @click="submit" :disabled="form.processing"
                class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50">Enregistrer</button>
        </div>
    </Modal>
</template>